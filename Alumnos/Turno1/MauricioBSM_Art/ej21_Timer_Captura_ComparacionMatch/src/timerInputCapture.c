...
bool_t timer0CaptureCallback( void* ptr ){
   ...
   if( calculated == FALSE ){
      ...
      result = ( tCapturadoActual - tCapturadoAnterior ) / (timerGetClock( TIMER0)/1000);
      tCapturadoAnterior = tCapturadoActual;
      
      consolePrintString( "F_CPU[MHz]: " );
      consolePrintlnInt( SystemCoreClock/1000000 );
      
      consolePrintString( "Prescale_Timer1: " );
      consolePrintlnInt( timerGetPrescale( TIMER1) );

      consolePrintString( "Prescale_Timer0: " );
      consolePrintlnInt( timerGetPrescale( TIMER0) );
      
      consolePrintString( "Clock_Timer0: " );
      ...
      consolePrintlnInt( 10000/result );
      consolePrintEnter();
   }
}
